...
mod tests {
    ...
    fn temp_root() -> PathBuf {
        std::env::temp_dir().join(format!("mesh-llm-log-foundation-{}", std::process::id()))
    }
    ...
    fn init_unwritable_root_fails_open() {
        let root = temp_root().join("unwritable-test");
        std::fs::create_dir_all(&root).unwrap();

        // On Unix, make the directory unwritable. Skip on Windows where chmod is unreliable.
        #[cfg(unix)]
        {
            ...
            // Remove all permissions to simulate unwritable.
            let mut new_perms = perms.clone();
            new_perms.set_mode(0o000);
            if std::fs::set_permissions(&root, new_perms).is_ok() {
                let foundation = LoggingFoundation::init(true, Some(&root));
                assert!(!foundation.is_healthy(), "unwritable root must fail open");

                // Restore permissions for cleanup.
                let _ = std::fs::set_permissions(&root, PermissionsExt::from_mode(perms.mode()));
            } else {
                // If we can't set 0o000 (e.g., root user), skip this assertion.
                tracing::debug!("skipping unwritable test: cannot set mode to 0o000");
            }
        ...
        #[cfg(not(unix))]
        {
            // On non-Unix, we can't easily make a dir unwritable. Just verify the happy path works.
            let foundation = LoggingFoundation::init(true, Some(&root));
            assert!(foundation.is_healthy());
            std::fs::remove_dir_all(&root).ok();
        }
